...
func _process(_delta):
	...
	$Control/Start/Countdown.text = str(ceil(st.time_left)) # this is the timer that goes 3 2 1 at the begining
	...
	$Control/TimePanel/TimeHBox/Minutes.text = str(m) + ":"
	$Control/TimePanel/TimeHBox/Seconds.text = str(s) if s >= 10 else "0" + str(s) # ternary. var x = (value) if (condition) else (value). this is so that when the number is less than 10 it will say 0:03
